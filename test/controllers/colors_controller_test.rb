...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { color: @color.color, name: @color.name, pantone: @color.pantone, year: @color.year } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { color: @color.color, name: @color.name, pantone: @color.pantone, year: @color.year } }
